...
To compile the code,
Run : g++ AA_A3_01FB16ECS450.cpp -Wall -g -o Test.out 

To Run the code,
Run : ./Test.out < InputFile.txt
...
1. Relevance score for every document was initialised to 0.
2. The Tree is first queried for the string (inclusive of Punctuations) and if found, The document's relevance score is incremented by TWICE the String length + 1.
3. The string is then stripped of all Punctions and is searched for again. If found, The document's relevance score is incremented by TWICE the String length. 
4. The string was split into individual words.
5. Each time a word is found, The document's relevance score is incremented by the String length of that word. 
6. The String is then searched for in the Title and if Found, The document's relevance score is incremented by HALF the String length. 
